  ...
  if (!party) {
    return (
      <div className="text-center py-12">
        ...
        <button onClick={() => navigate('/crm')} className="mt-4 text-emerald-600 hover:underline">
          Voltar para CRM
        </button>
      </div>
    );
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <textarea 
                    ...
                    className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 outline-none resize-none"
                    rows={2}
                  />
                </div>
            ...
            <div className="space-y-3">
              {relatedProcesses.map(proc => (
                <Link 
                  ...
                  className="block p-3 bg-slate-50 rounded-xl border border-slate-100 hover:border-emerald-200 hover:bg-emerald-50/30 transition-colors"
                >
                  <div className="font-medium text-emerald-600 text-sm">{proc.Numero_Processo}</div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col h-[600px]">
          ...
          <div className="shrink-0 mt-auto pt-4 border-t border-slate-100 space-y-3">
            ...
            <input
              ...
              className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 outline-none text-slate-600"
            />
            <button
              onClick={handleAddTask}
              disabled={!newTaskTitle.trim() || !newTaskDate}
              className="w-full flex items-center justify-center gap-2 py-2.5 bg-emerald-600 text-white rounded-xl font-medium hover:bg-emerald-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <Save size={16} />
              Adicionar Tarefa
            </button>
